...
N = 10000;
x = linspace(-pi, pi, N);
a0 = integral(fx, -pi, pi, 'AbsTol', 1e-12, 'RelTol', 1e-12)/pi;
display(a0);
a = zeros(1, D);
b = zeros(1, D);
for k = 1:D
    a(k) = integral(@(x) fx(x).*cos(k*x), -pi, pi, 'AbsTol', 1e-12, 'RelTol', 1e-12)/pi;
    b(k) = integral(@(x) fx(x).*sin(k*x), -pi, pi, 'AbsTol', 1e-12, 'RelTol', 1e-12)/pi;
end
display(a);
display(b);
